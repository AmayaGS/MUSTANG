...
#from torch.geometric.nn.profile

from torch_geometric.utils import to_networkx, assortativity, to_undirected, contains_isolated_nodes
...
class GCN_model(torch.nn.Module):
    ...
    def __init__(self, dim_in, heads=8, pooling_ratio=0.5):
        ...
        self.gat1 = GCNConv(dim_in, 512, heads=self.heads, concat=False)
        self.gat2 = GCNConv(512, 512, heads=self.heads, concat=False)
        self.gat3 = GCNConv(512, 512, heads=self.heads, concat=False)
        self.gat4 = GCNConv(512, 512, heads=self.heads, concat=False)
...
class GAT_SAGPool_flops_1(torch.nn.Module):
    ...
    def __init__(self, dim_in, heads=1, pooling_ratio=0.5):
        ...
        self.gat1 = GATv2Conv(dim_in, 512, heads=1, concat=False)
        self.gat2 = GATv2Conv(512, 512, heads=1, concat=False)
        self.gat3 = GATv2Conv(512, 512, heads=1, concat=False)
        self.gat4 = GATv2Conv(512, 512, heads=1, concat=False)
    ...
    def forward(self, x):
        ...
        knn_graph = kneighbors_graph(x.squeeze(0).cpu(), 5, mode='connectivity', include_self=False)
        edge_index = torch.tensor(np.array(knn_graph.nonzero()),dtype=torch.long)
        data = Data(x=x, edge_index=edge_index)
