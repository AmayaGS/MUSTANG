...
class GatedAttention_flops(nn.Module):
    ...
    def __init__(self, L= 1024, D=224, Dropout=True, n_classes=2, k_sample=8, instance_loss_fn=nn.CrossEntropyLoss(), subtyping=False):
        ...
        super(GatedAttention_flops, self).__init__()
        self.L = L
        self.D = D
        self.Dropout= Dropout
        self.n_classes = n_classes
    ...
    def forward(self, x, label=None, instance_eval=True):
        ...
        #     all_preds = []
        #     all_targets = []
        #     inst_labels = F.one_hot(torch.arange(0, 1), 2).squeeze()
        #     #inst_labels = F.one_hot(label, num_classes=self.n_classes).squeeze() #binarize label
        #     for i in range(len(self.instance_classifiers)):
        #         inst_label = inst_labels[i].item()
        #         classifier = self.instance_classifiers[i]
        ...
        M = torch.mm(A, x) 
        #M = torch.mm(A.squeeze(1).squeeze(1).unsqueeze(0), x.squeeze(0)) # for get_model_complexity package
    ...
    def inst_eval(self, A, x, classifier):  # h=x
        ...
        logits = classifier(all_instances)
        all_preds = torch.topk(logits, 1, dim = 1)[1].squeeze(1)
        instance_loss = self.instance_loss_fn(logits, all_targets.unsqueeze(0))
        return instance_loss, all_preds, all_targets
